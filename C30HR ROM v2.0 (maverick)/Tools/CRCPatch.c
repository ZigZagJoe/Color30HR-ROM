...
ZZJ 2024 changed code back to decl because MPW disallows -rt CODE
  ...
  if (argc == 1)
    {
      ...
      fprintf(stderr,"### DSCRPT: Calculate and patch the crc value to decl 1.\n");
    }
  else if (argc != 2)
    {
  ...
  else
    {
	  ...
	  if (refnum < 0 )
	    ...
	    {
		   CodeHandle = Get1Resource('decl',1);
		   
		   if (CodeHandle == nil)
             fprintf(stderr,"### ERROR : Decl resource not available. Err = %d\n",errno);
		   else
		     {
